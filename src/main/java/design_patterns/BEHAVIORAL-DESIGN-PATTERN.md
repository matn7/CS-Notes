...
- In template a complicated algorithm is implemented usually in the form of an instantiated method of an
abstract base class.
...
- UI Applications are often built on the composite pattern, windows contain other windows.
- The nested windows pass the mouse click action down from one to another
