...
- Add a element to the End of the Queue and remove elements from the beginning of the queue. FIFO, LIFO.
...
- Stream is split and then each part are processed and then join.
...
- Java 8 Streams is based on convert collection to streams.
...
- Permanent generation is used to store metadata. Store String pool as well.
...
- Generation are then separated on 3 parts:
    ...
    - Survivor 1.
    - Survivor 2.
- A new object is then created on a heap in new generation in **Eden** space, and after few GC it goes to **Survivor1**
...
- View are not virtually stored, and it takes less space to store.
...
- It's a generic class that explicitly returns the thread results. 
