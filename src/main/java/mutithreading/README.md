...
- If a thread finished its work, but the application is still running, we want to clean up the
thread's resources
...
- Tread may take unreasonably long time. Always use Thread.join() with a time limit
...
- If our calling hiererchy is too deep. We may get an StackOverflow Exception.
...
system as if it occured at once
...
- Items++ - Not an atomic operation:
    ...
    - CurrentValue :left_arrow: 0
    - newValue :left_arrow: currentValue + 1 = 1
    - Items :left_arrow: newValue = 1
...
- Both threads are reading and modyfying that counter in the same time
...
- Atomic operations
    ...
    - Assignemnts to references
    - Assignments to double and long using volatile keyword

### Race condition

- Condition when multiple threads are accessing a shared resources
- At least one thread is modyfying the resource
- The timing of threads schedulting may cause incorrect results
...
- Synchronized and ReentrantLock do not allow multiple readers to access a shared resource
concurrently
...
- If the condition is not met - thread A goes to sleep until another thread changes the semaphore's
state
...
- The lock ensures atomic check and modification of the shared variables, involved in
the condition
...
- Samaphore as a particular case of condition variable
- COndition Variable
...
- High Priority thread cannot progress because of the low priority thread is not scheduled to release
the lock
...
- A real problem for latence sensitive, high speed trading systems
...
- Utilize operations which are guaranteed to be one hardwareoperation
...
- Pros
    ...
    - No tace conditions or data races
- Cons
...
- If used only by a single thread, a regular integers is prefered
